...
int RoverArmMotor::forward(int percentage_speed)
{
    ...
    {
        this->disengage_brake();
        float duty_cycle = BLUE_ROBOTICS_STOP_DUTY_CYCLE + (BLUE_ROBOTICS_STOP_DUTY_CYCLE * ((float) percentage_speed / 100.0f)); // TODEBUG
...
int RoverArmMotor::reverse(int percentage_speed)
{
    ...
    {
        this->disengage_brake();
        float duty_cycle = BLUE_ROBOTICS_STOP_DUTY_CYCLE - (BLUE_ROBOTICS_STOP_DUTY_CYCLE * ((float) percentage_speed / 100.0f));
        pwmInstance->setPWM(_pwm, _pwm_freq, duty_cycle);
        return 0;
    }
    return -1;
